...
def gray_encode( bn ):
    ...
    return int(bn) ^ int( bn / 2 )

def gray_decode( n ):
    sh = 1
    while True:
        div = shift_right_float(n, sh)
        n = xor_float(n,div)
        if div <= 1: return n
...
def gray_encode_travel( start, end, mask, i ):
    travel_bit = xor_float(start,end)
    ...
    return xor_float((and_float(or_float(g,( g / modulus )), mask)), start)

def gray_decode_travel( start, end, mask, g ):
    travel_bit = xor_float(start,end)
    modulus = mask + 1          # == 2**nBits
    rg = ( xor_float(g,float(start)) ) * ( modulus / ( travel_bit * 2 ) )
    return gray_decode(and_float(or_float(rg,(rg / modulus)), mask))
...
def child_start_end( parent_start, parent_end, mask, i ):
   start_i = max( 0,    and_float(( i - 1 ), ~1) )  # next lower even number, or 0
   end_i =   min( mask, or_float(( i + 1 ),  1) )  # next higher odd number, or mask
